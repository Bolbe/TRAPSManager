...
void DumpController::service(stefanfrings::HttpRequest& request, stefanfrings::HttpResponse& response)
{

    response.setHeader("Content-Type", "text/html; charset=ISO-8859-1");
    response.setCookie(stefanfrings::HttpCookie("firstCookie","hello",600));
    response.setCookie(stefanfrings::HttpCookie("secondCookie","world",600));
    ...
    QMapIterator<QByteArray,QByteArray> j(request.getCookieMap());
    while (j.hasNext())
    {
        j.next();
        body.append("<br>");
