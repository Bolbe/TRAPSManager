...
void CompetFFCKConnector::connectToServer(const QString& host, int port) {
    ...
    if (!_socket->waitForConnected(5000)) {
        ...
        emit error("Problème de connexion CompetFFCK", QString("Impossible de se connecter à CompetFFCK sur\n%0:%1\nVérifiez le numéro IP et le port d'écoute de CompetFFCK.\nVérifiez que le mode gestion des pénalités est démarré.").arg(host).arg(port));
        _socket->deleteLater();
        return;
    }
    _connected = true;
...
void CompetFFCKConnector::sendPenalty(QPair<int,int> bib, int gateId, int penalty) {
    ...
    QByteArray byteArray(QString("penalty %1 %2 %3 %4\r")
                         .arg(bib.first)
                         .arg(gateId)
                         .arg(bib.second+1)
                         .arg(penalty).toLocal8Bit());
