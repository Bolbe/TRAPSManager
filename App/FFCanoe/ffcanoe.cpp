...

{
    ...
    QSettings settings;
    _host = settings.value(FFCANOE_HOST, "102.168.1.x").toString();
...
void FFCanoe::sendPenalty(QPair<int,int> bib, int gateId, int penalty) {
    if (_forwardPenalty && _connected) {
        _buffer++;
        emit bufferChanged(_buffer);
        emit sendPenaltyToServer(bib.first, gateId, penalty);
    }
}
