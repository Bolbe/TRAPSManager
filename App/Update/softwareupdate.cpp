...

{
    ...
    QSettings settings;
    _checkAtStartup = settings.value("checkAtStartup", QVariant(true)).toBool();
...
QString SoftwareUpdate::appTitle() const {
    return QString("TRAPS Manager %0").arg(_appVersion);
}
...
void SoftwareUpdate::checknewVersion(bool force) {
    ...
    ->onResult([this](QJsonDocument doc) {
        ...
        if (timestamp>_appTimestamp) {
            ...
            QString filename = _target.mid(_target.lastIndexOf('/')+1);
...
void SoftwareUpdate::onError() {
    auto code = _reply->error();
    if (!_downloading) {
...
void SoftwareUpdate::download() {
    ...
    connect(_reply, &QNetworkReply::error, this, &SoftwareUpdate::onError);
    _downloading = true;
    emit progress(0);
    emit downloadingChanged(_downloading);
