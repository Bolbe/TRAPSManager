...
function device.GetInformation()
	return { version = 3.0, name = 'Serveur TRAPS', class = 'network' };
end	
...
function device.OnConfiguration(node)
	...
	local dlg = wnd.CreateDialog({
		...
		label = "Configuration Gestion des Penalites",
		width = 300,
		height = 200
	});
...
function AddChrono(bib, chrono)
	app.SendNotify("<bib_time>", 
		{ time_chrono = chrono, bib = bib  }
	);
end
